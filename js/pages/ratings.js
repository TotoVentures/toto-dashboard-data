...
const RatingsPage = (() => {
  ...
  function render(container, data, filterState) {
    ...
    // Count apps by rating tier
    const apps4plus = rows.filter(r => r.rating >= 4.0).length;
    const appsBelow3 = rows.filter(r => r.rating > 0 && r.rating < 3.0).length;

    // KPI cards
    const metrics = [
      { label: 'Total Ratings', field: 'total', value: totalRatings, isCurrency: false, description: 'Sum of all ratings across all apps (worldwide)' },
      { label: 'Avg Rating', field: 'avg', value: avgRating, isCurrency: false, description: 'Weighted average across portfolio' },
      { label: 'Apps Rated 4+', field: 'good', value: apps4plus, isCurrency: false, description: 'Number of apps with 4.0+ average rating' },
      { label: 'Apps Below 3.0', field: 'bad', value: appsBelow3, isCurrency: false, description: 'Number of apps with sub-3.0 rating — may need attention' }
    ];
